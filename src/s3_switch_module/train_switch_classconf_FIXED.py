...
df = pd.read_csv('h264_switch_training_data_with_class_signal_FIXED.csv')
...
print("\n" + "=" * 62)
print("HELD-OUT EVAL (training-set split) — LEAK FIXED")
print("=" * 62)
for thr in [0.3, 0.4, best_thr, 0.6, 0.7]:
    ...
    print(f"{thr:.2f}  bal-acc={bal:.4f}  catch-SAR={rec_pos:.4f}  keep-CAR={rec_neg:.4f}  %to-SAR={100*frac_sar:.1f}%{mark}")
...
torch.save({'state_dict': model.state_dict(), 'scaler_mean': scaler.mean_,
           ...
           'switch_mlp_classconf_FIXED.pth')
print("\nsaved switch_mlp_classconf_FIXED.pth")
