...
FEATURES = ['mv_mean', 'mv_std', 'mv_max', 'mv_sparsity',
            'residual_energy_mean', 'num_frames']

df = pd.read_csv('switch_training_data.csv')
...
print(f"\npos_weight = {pos_weight.item():.2f}  "
      f"(up-weighting the minority 'needs SAR' class)\n")
...
# Select operating threshold on VALIDATION, never on eval
model.eval()
with torch.no_grad():
    val_probs = torch.sigmoid(model(X_val_t))
...
print(f"selected threshold: {best_thr:.2f} "
      f"(val balanced-acc {best_thr_score:.4f})")

# Final, single evaluation on the held-out set
with torch.no_grad():
...
print("HELD-OUT EVAL")
print("=" * 62)
print(f"{'thr':>5} {'bal-acc':>8} {'catch-SAR':>10} {'keep-CAR':>9} "
      f"{'%to-SAR':>8}")
for thr in [0.3, 0.4, best_thr, 0.6, 0.7]:
    ...
    print(f"{thr:5.2f} {bal:8.4f} {rec_pos:10.4f} {rec_neg:9.4f} "
          f"{100*frac_sar:7.1f}%{mark}")
...
print(f"  clips kept on CAR : {100*kept:.1f}%")
print(f"  clips sent to SAR : {100*frac_sar:.1f}%")
print(f"  of clips kept on CAR, CAR was correct: "
      f"{100*car_right_when_kept/max(1,(eval_probs < best_thr).sum().item()):.1f}%")
print(f"  (baseline: CAR correct on {100*(y_eval_t==0).float().mean():.1f}% "
      f"of all clips)")
...
torch.save({'state_dict': model.state_dict(),
            ...
            'features': FEATURES,
            'threshold': float(best_thr)}, 'switch_mlp.pth')
print("\nsaved switch_mlp.pth")