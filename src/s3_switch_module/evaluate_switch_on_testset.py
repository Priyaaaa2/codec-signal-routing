"""
evaluate_switch_on_testset.py

The final, honest evaluation. Uses ONLY test-set data (3,783 clips) and the
REAL deployed models (full-dataset CAR, full-dataset SAR/I3D) — no fold
models, no training-set numbers mixed in.

The class fail-rate lookup is the one artifact allowed to come from training
data, since it's a fixed, pre-computed parameter, not a per-clip prediction.
"""
...
motion_feats = pd.read_csv('features/all_features_v2.csv')
class_lookup = pd.read_csv('class_fail_rate_lookup.csv', index_col=0)['fail_rate'].to_dict()

df = car.merge(i3d, on='clip_path', how='inner').merge(motion_feats, on='clip_path', how='inner')
...
ckpt = torch.load('switch_mlp_v2.pth', map_location='cpu', weights_only=False)
...
print("=" * 60)
print("FINAL, HONEST TEST-SET EVALUATION (3,783 clips)")
print("=" * 60)
print(f"Always-CAR baseline accuracy: {always_car_acc*100:.2f}%")
print(f"Always-SAR baseline accuracy: {always_sar_acc*100:.2f}%")
print()
...
print()
print(f"Router catch rate (fraction of CAR's real failures sent to SAR): {catch_rate*100:.2f}%")
print()
print(f"FINAL SYSTEM ACCURACY: {system_accuracy*100:.2f}%")
print(f"(Cannot exceed SAR's {always_sar_acc*100:.2f}% by construction)")

df['route_to_sar'] = route_to_sar
df['switch_prob'] = probs
df['final_correct'] = final_correct
df.to_csv('final_test_evaluation.csv', index=False)
