...
FEATURES = ['predicted_class_fail_rate_loo', 'car_confidence',
            ...
            'av1_mv_mean', 'av1_mv_std', 'av1_mv_max', 'av1_mv_sparsity', 'av1_fine_partition_frac']

df = pd.read_csv('combined_h264_av1_switch_training_data.csv')
...
for epoch in range(1, 301):
    ...
    if val_bal > best_val:
        ...
        since_best += 1
    if epoch % 25 == 0:
        print(f"epoch {epoch:3d}  val balanced-acc {val_bal:.4f}")
...
print(f"\nHeld-out eval: bal-acc={bal:.4f} catch-SAR={rec_pos:.4f} keep-CAR={rec_neg:.4f} pct-to-SAR={100*frac_sar:.1f}%")
...
torch.save({'state_dict': model.state_dict(), 'scaler_mean': scaler.mean_,
           ...
           'switch_mlp_v3_with_av1.pth')
print("saved switch_mlp_v3_with_av1.pth")
